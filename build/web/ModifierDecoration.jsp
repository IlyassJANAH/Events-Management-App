...
<!-- ********************************************          MENU             **************************************************** -->
               ...
               <nav id="page-leftbar" role="navigation" >
                ...
                <li ><a href="CalendrierAdmin.jsp"><i class="fas fa-calendar-alt fa-lg pad"></i> <span>Calendrier des demandes</span></a> </li>
                 <li class="divider"></li>
                        ...
                        <li class="active"><a href="Decoration.jsp"><i class="fas fa-lightbulb fa-lg pad"></i> <span>Decoration</span></a> </li>
   ...
   <sql:setDataSource var = "test" driver = "com.mysql.jdbc.Driver"
        ...
        <sql:query dataSource = "${test}" var = "result">
            SELECT * from decoration WHERE idDecoration=<%= request.getParameter("idDeco") %>
         </sql:query>
    <!-- *********************************   Fin Requete *         *********************************************** -->
...
<div class="panel panel-midnightblue">
    <div class="panel-heading">
        <h4>Modifier Décoration</h4>
        
    </div>
    <div class="panel-body collapse in">
        <form action="ModifierDecoration" method="post" class="form-horizontal row-border">
            <c:forEach var = "row" items = "${result.rows}">
                <input type="hidden" name="id" value="<c:out value = "${row.idDecoration}"/>" ><!-- type hidden pour cacher l id et l'utilise pour update-->
  <div class="form-group">
                <label class="col-sm-3 control-label">Libellé</label>
    <div class="col-sm-6">
                <input type="text" name="Libelle" placeholder="Entrer un libellé" value="<c:out value = "${row.LibelleDecoration}"/>" required>
    </div>
    </div>
  ...
  <div class="form-group">
     <label class="col-sm-3 control-label">Désignation</label>
         <div class="col-sm-6">
     <textarea  name="Designation" id="exampleFormControlTextarea1" rows="3" required><c:out value = "${row.DesignationDecoration}"/></textarea>
         </div>
         </div>
  ...
  <div class="form-group">
                ...
                <input type="number" step="0.01" name="prix" class="form-control bor" placeholder=" Veuillez Entrer un prix" value="<c:out value = "${row.PrixDecoration}"/>" required>
                <br> <small class="form-text text-muted">Entrer le prix en Dirhams</small> </div>
  </div>
...
function Confirmer(id) {
  var txt;
  var r = confirm("Etes-vous sur de vouloir supprimer cette Decoration ?");
  if (r == true) {
    window.location.href = "http://localhost:8080/Projet_test/SupprimerDecoration?idDeco="+id;
  } else {
