...
<div id="page-container">
   ...
   <sql:setDataSource var = "test" driver = "com.mysql.jdbc.Driver"
        ...
        <sql:query dataSource = "${test}" var = "result">
            SELECT * from animation WHERE idAnimation=<%= request.getParameter("idAnim") %>
         </sql:query>
    <!-- *********************************   Fin Requete *         *********************************************** -->
...
<div class="panel panel-midnightblue">
    <div class="panel-heading">
        <h4>Modifier Animation</h4>
        
    </div>
    <div class="panel-body collapse in">
        <form action="ModifierAnimation" method="post" class="form-horizontal row-border">
            <c:forEach var = "row" items = "${result.rows}">
                <input type="hidden" name="id" value="<c:out value = "${row.idAnimation}"/>" ><!-- type hidden pour cacher l id et l'utilise pour update-->
  <div class="form-group">
    <label class="col-sm-3 control-label">Libellé</label>
    <div class="col-sm-6">
       <input type="text" style="width:300px;" name="Libelle" placeholder="Entrer un libellé" value="<c:out value = "${row.LibelleAnimation}"/>" required>
    </div>
  </div>
    <div class="form-group">
       <label class="col-sm-3 control-label">Désignation</label>
      <div class="col-sm-6">
    <textarea name="Designation" id="exampleFormControlTextarea1" rows="4" required><c:out value = "${row.DesignationAnimation}"/></textarea>
      </div>
   </div>    
  ...
  <div class="form-group">
                ...
                <div class="col-sm-6">
<input type="number" step="0.01" name="prix"  placeholder=" Veuillez Entrer un prix" value="<c:out value = "${row.PrixAnimation}"/>" required>
<small  class="form-text text-muted">Entrer le prix en Dirhams</small>                    </div>
  ...
  <div class="modal-footer" style="text-align: left;"> 
      ...
      <button type="button" class="btn btn-secondary" data-dismiss="modal"><a href="Animation.jsp" style="color: #333333;">Annuler</a></button>
      </div>
  </c:forEach>
    </form>
